class Table
    ...
    def determine_winner
        ...
        if self.players[1].folded == true || p1_hand > p2_hand
            ...
            self.players[0].chipstack+=self.pot
            ...
            self.players[1].chipstack+=self.pot
            ...
            self.players[1].chipstack+=self.pot / 2
            self.players[1].chipstack+=self.pot / 2
            unless self.pot % 2 == 0
                rand(1..2) == 1 ? self.players[0].chips+= 1 : self.players[1].chips+= 1
            end 
        end
    end
    ...
    def remaining_players?
        return nil if self.players.any? { |p| p.chipstack == 0 }
        return nil if self.players.any? { |p| p.folded == true }
        true
    end
